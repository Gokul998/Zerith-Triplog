...
router.get("/", requireAuth, async (req, res) => {
  const expenses = await query(`SELECT e.*, u.name as paid_by_name, u.avatar_color as paid_by_color FROM expenses e JOIN users u ON e.paid_by = u.id WHERE e.trip_id = ? ORDER BY e.date DESC, e.created_at DESC`, [req.params.tripId]);
  res.json((expenses as any[]).map((e: any) => ({ ...e, split_among: JSON.parse(e.split_among) })));
});
...
router.post("/", requireAuth, async (req, res) => {
  ...
  await execute("INSERT INTO expenses (id, trip_id, paid_by, title, amount, currency, category, split_among, date, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [id, req.params.tripId, userId, title, amount, currency, category, JSON.stringify(split_among), date, notes]);
  const expense = await queryOne(`SELECT e.*, u.name as paid_by_name, u.avatar_color as paid_by_color FROM expenses e JOIN users u ON e.paid_by = u.id WHERE e.id = ?`, [id]) as any;
...
router.get("/balances", requireAuth, async (req, res) => {
  const expenses = await query(`SELECT e.*, u.name as paid_by_name FROM expenses e JOIN users u ON e.paid_by = u.id WHERE e.trip_id = ?`, [req.params.tripId]) as any[];
  ...
  const members = await query(`SELECT u.id, u.name FROM trip_members tm JOIN users u ON tm.user_id = u.id WHERE tm.trip_id = ? UNION SELECT u.id, u.name FROM trips t JOIN users u ON t.owner_id = u.id WHERE t.id = ?`, [req.params.tripId, req.params.tripId]) as any[];
