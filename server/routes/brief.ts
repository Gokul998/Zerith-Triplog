...
router.get("/", requireAuth, async (req, res) => {
  ...
  const trip = await queryOne(`SELECT t.*, u.name as owner_name FROM trips t JOIN users u ON t.owner_id = u.id WHERE t.id = ?`, [tripId]) as any;
  ...
  const activities = await query(`SELECT * FROM activities WHERE trip_id = ? ORDER BY day_id ASC, sort_order ASC`, [tripId]) as any[];
  ...
  // Checklist items
  const checklist = await query(`SELECT * FROM checklist_items WHERE trip_id = ? ORDER BY category ASC, created_at ASC`, [tripId]) as any[];
  ...
  const members = await query(`
    ...
    WHERE tm.trip_id = ?
  `, [tripId]) as any[];
  const owner = await queryOne("SELECT name, email FROM users WHERE id = ?", [trip.owner_id]) as any;
